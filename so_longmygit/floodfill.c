...
/*   Updated: 2026/04/22 09:48:22 by zernest          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	flood_fill(t_mlx **mlx)
{
	...
	if ((*mlx)->ex_state == 0 || remaining_c(map_dup) != 0)
	{
		free_data(&map_dup, &player_loc);
		exit_err_str(mlx, "Error\nInvalid map (unreachable exit or collectibles)");
	}
	free_data(&map_dup, &player_loc);
}
